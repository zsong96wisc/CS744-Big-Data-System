...
conf = SparkConf().setAppName("pagerank_task3_disk")
...
link2EachDest = rawData.map(lambda row: (row.split("\t")[0], row.split("\t")[1])).persist(storageLevel=StorageLevel(True,False,False,False,1))
